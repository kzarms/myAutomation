...
Describe "Basic user Display name and Mail address tests" {
    Context "Test CreateUserDisplayName function" {
        ...
        It "DisplayName with special charts" {
            CreateUserDisplayName -FirstName "Alice " -LastName "B�ris" | Should -Be "Alice B�ris (External RAS/VDI)"
            }
        It "Fail test Wrong creation" {
            CreateUserDisplayName -FirstName "Alice " -LastName "B�ris" | Should -Not -Be "Alice Boris (External RAS/VDI)"
            }  
      }
